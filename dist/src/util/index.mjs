...
export { Constants };
const { errors: { command: CommandErrors }, } = Constants;
export function getVariableType(data) {
    return new Type(data).toString();
}
export function resolveErrorType(type) {
...
export function resolveErrorMessage(data) {
    const { type: RawType, name, expected: _expected, received: _received, message: Message, } = data, type = resolveErrorType(RawType), expected = typeof _expected === "string"
        ? _expected
        : new Type(_expected).toString(), received = typeof _received === "string"
        ? _received
