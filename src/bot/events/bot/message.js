...
import blwords from "../../collections/blwords.js";
import leven from "../../utilities/leven.js";
import Cooldown from "../../schemas/cooldown.js";
import BaseEvent from "../../structures/internal/BaseEvent.js";
const { blacklistedWords } = blwords;
...
async function handleCooldown(message, command) {
    ...
    if (cd) {
        ...
        if (cd.expiration < now) {
            ...
            return message.sendErrorReply("Chillza.", `You need to wait ${Math.floor((cd.expiration - now) / 1000).toFixed(1)} more second(s) before reusing the \`${command.name}\` command.`, `"Patience is the key my child."`);
        }
    }
    return false;
}
async function makeCooldown(message, command) {
    ...
    await Cooldown.create({
        key: generateKey(message.author.id, command.name),
        expiration: Date.now() + (command.cooldown ?? 3) * 1000,
    });
}
    ...
    async run(message, client) {
        ...
        if (!message.channel
            ...
            return message.author
                .sendError(message, "Invalid Permissions!", "I don't have enough permissions in this guild! Please ask an admin to give me the following permissions: \n `READ_MESSAGES`, `SEND_MESSAGES`, `EMBED_LINKS`")
                ?.catch(() => { });
        //------------------------------------------------------------------
        ...
        if (!userSchema)
            userSchema = await client.db.users.fetch(author.id);
        ...
        blacklistedWords.map((x) => {
            if (message.content.toLowerCase().includes(x.toLowerCase()) &&
                !message.author.bot &&
                ...
                if (difference > DIFF) {
                    ...
                    if (parseInt(msgCount) === LIMIT) {
                        const spamEmbed = new MessageEmbed()
                            ...
                            .setColor("RED")
                            .setDescription("**Chillza!** Hey, would you mind slowing down? You've sent 5 messages in 5 seconds.")
                            .setThumbnail("https://assets.onlinelabels.com/images/clip-art/Leomarc/Leomarc_stop_sign.png");
                        ...
                        setTimeout(async () => {
                            ...
                            const unmuteEmbed = new MessageEmbed()
                                ...
                                .setColor("GREEN")
                                .setDescription("You have been unmuted now, better not spam next time!")
                                .setThumbnail("https://lh3.googleusercontent.com/proxy/sz_ww5-B0qhs7RPhI7ilQ6Wq06IFvw7aGCl30oqn4KduUYdMz3ElboKF911VVWO0QYwodKSH3p5eEKECTvOQFcsPQeMQ4m0");
        ...
        const whichToUse = prefixes?.length == 1
            ? guildSchema.prefixes.toString()
            : "{prefix/mention}";
        const embed = new MessageEmbed()
            .setAuthor("Hello, I'm Hurricano™!", "https://raw.githubusercontent.com/HurricanoBot/HurricanoImages/master/SetAuthorEmojis/Wave.png")
            .addField("Forgot my prefix?", `No worries! My ${guildSchema.prefixes?.length > 1
            ? "prefixes are"
            : "prefix is"} ${prefixes}.`)
            .addField("Need help?", `Oh! Well, incase you want to see my commands, you can just use \`${whichToUse}help\`, and incase you need a more detailed description, you can use \`${whichToUse}help {command name}\``)
            .addField("Want to view my info?", `Cool! You can do so by using \`${whichToUse}botinfo\`. By the way, we also support slash commands!`)
            .addField("Still need help?", "Feel free to join the Hurricano™ support server, by clicking [here](https://discord.gg/RDEBGXp7sG)!")
            .setColor("#034ea2")
            .setImage("https://raw.githubusercontent.com/HurricanoBot/HurricanoImages/master/other/Wave.png")
            .setFooter(`© Hurricano™ v1.0.0`);
        if (prefixRegex.test(message.content.toLowerCase())) {
            if (userSchema.blacklisted)
                return message.sendErrorReply("You have been blacklisted!", "Damn it! You have been blacklisted by a bot moderator! This means you will be unable to use any of the bot commands.");
            if (guildSchema.blacklisted)
                return message.sendErrorReply("Blacklisted!", "This guild has been blacklisted. It cannot use any commands.");
            if (message.content === `<@${client.user.id}>` ||
                (message.content === `<@!${client.user.id}>` &&
                    !userSchema.blacklisted))
                return message.channel.send({ embeds: [embed] });
            const [, match] = message.content.toLowerCase().match(prefixRegex);
            if (!message.content.toLowerCase().startsWith(match))
                return;
            ...
            if (!command) {
                ...
                ].filter((c) => leven(cmd.toLowerCase(), c.toLowerCase()) <
                    c.length * 0.4);
                const dym = best.length == 0
                    ? ""
                    : best.length == 1
                        ? `+ ${best[0]}`
                        : `${best
                            ...
                            .join("\n")}`;
                return dym
                    ? message.channel.sendError(message, "Invalid Command!", `Sorry! I don't have that command! Did you happen to mean: \n\`\`\`diff\n${dym}\`\`\``)
                    : null;
            }
            ...
            if (command.subCommands && args.length) {
                ...
                const subCommand = subCommands.commands.get(args[0]) ||
                    subCommands.commands.find((x) => {
                        const keyfunc = getKey(x);
                        if (Array.isArray(keyfunc) && keyfunc.includes(args[0]))
                            return x;
                    });
                if (subCommand) {
                    /**
                    ...
                    if (subCommands.baseAuthorization) {
                        const auth = subCommands.baseAuthorization
                            ? await subCommands.baseAuthorization(message, args, subArgs, args[0])
                            : null;
                        if (auth && auth !== "allow")
                            return;
                    }
            ...
            if (command.ownerOnly === true && !checkAdmin)
                return message.channel.sendError(message, "Permission Error.", `You are not the owner of Hurricano™, ${author}.`);
            if (!message.member)
            ...
            if (command.userPermissions) {
                if (!checkPerm(command.userPermissions) &&
                    !client.config.ownerIds.includes(message.author.id)) {
                    return message.sendErrorReply("Permission Error.", `Stop disturbing me bro, you require the \`${command.userPermissions.join(", ")}\` permission(s) to use that command...`, "Smh, imagine trying to use a command without having the perms-");
                }
            }
            if (command.args && !args.length)
                return message.channel.sendError(message, "Arguments Error.", command.args);
            if (disabledModules && !disabledModules.includes("levelling")) {
                const userLevel = await client.levels.fetch(message.author.id, message.guild.id);
                if (!userLevel)
                ...
                if (hasLeveledUp) {
                    ...
                    await message.sendSuccessReply(`Level Up!`, `${message.author}, congratulations! You have leveled up to **${user.level}**! :tada:`);
                }
            }
