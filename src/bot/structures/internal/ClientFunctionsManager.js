...
const { HurricanoClient } = client;
export default (class ClientFunctionsManager {
    /**
    ...
    createOptionsEmbed(command, message) {
        const optionsEmbed = new MessageEmbed()
            ...
            .addField("Permissions", command.userPermissions?.length
            ? command.userPermissions.map((x) => `\`${x}\``).join(", ")
            : "No user permissions required.")
            .addField("Subcommands:", `${command.subCommands}`)
            .setFooter(`Type ${message._usedPrefix}help <command> for more info on a command.`);
        return optionsEmbed;
    }
    async getMember(returnAuthor, message, ...args) {
        ...
        if (!message)
            throw new Error("Message object not provided.");
        if (!args.length)
            throw new Error("Arguments string not provided.");
        ...
        if (message.mentions?.members.first())
            ...
            user = await message.guild.members.fetch(args).catch(() => { });
        if (user && user.size)
            user = null;
        if (returnAuthor && !user)
    ...
    async getChannel(returnChannel, message, ...args) {
        ...
        if (typeof returnChannel !== "boolean")
            throw new Error(`Whether to return channel or not option is not boolean.`);
        if (typeof message !== "object")
            throw new Error(`Message provided is not an object.`);
        args = args[0];
        let user;
        if (message.mentions?.channels.first())
            ...
            user = message.guild.channels.cache.get(args);
        if (returnChannel && !user)
     ...
     */
    shuffle(content) {
        var a = content.split(""), n = a.length;
        for (var i = n - 1; i > 0; i--) {
