	...
	async loadTopgg() {
		if (this.config.topgg.enabled) {
			...
			if (this.config.topgg.webhook.enabled) {
				...
				const voteWebhook = new DBL.Webhook(
					this.config.topgg.webhookPassword,
				);
				...
				await app.post(
					...
					voteWebhook.listener(async (vote) => {
						...
						this.logger.info(
							`User with ID ${user.tag} just voted!`,
						);
				...
				const port =
					process.env.PORT || this.config.topgg.webhook.webhookPort;
