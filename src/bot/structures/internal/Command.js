...
export default class Command {
    ...
    static validateOpts(opts) {
        ...
        if (opts.aliases) {
            if ((this.aliases && !Array.isArray(opts.aliases)) ||
        ...
        if (opts.clientPermissions) {
            ...
            for (const perm of opts.clientPermissions) {
                if (!permissions[perm])
                    throw new RangeError(`Command: ${name}: Invalid command client permission(s): ${perm}`);
            }
        }
        if (opts.subCommands) {
            ...
            for (const sub of opts.subCommands.commands) {
                if (!Array.isArray(sub))
                    throw new Error(`Command: ${name}: One of the subcommands aren't an array.`);
            }
        }
        if (opts.userPermissions) {
            ...
            for (const perm of opts.userPermissions) {
                if (!permissions[perm])
                    throw new RangeError(`Command: ${name}: Invalid command user permission(s): ${perm}`);
            }
        }
