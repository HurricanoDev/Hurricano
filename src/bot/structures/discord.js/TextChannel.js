...
export const name = "TextChannel";
export const extend = moduleExports.extend;
const moduleExports = {
    name,
    extend({ TextChannel, bind }) {
        Object.defineProperties(TextChannel, bind({
            send: {
                value: async function send(optionsRaw) {
                    ...
                    let options = typeof optionsRaw === "string"
                        ? optionsRaw
            ...
            sendError: {
                value: async function sendError(message, Header, Msg, Footer, Fields, Components) {
                    ...
                    if (Components)
                        sendObj.components = Components;
                    const msg = await this.send(sendObj);
                    return msg;
                },
            },
            sendSuccess: {
                value: async function sendSuccess(message, Header, Msg, Footer, Fields, Components) {
                    const embed = new MessageEmbed()
