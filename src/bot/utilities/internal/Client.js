...
import Database from "../../handlers/db.js";
import extractor from "@discord-player/extractor";
import ClientCommandsManager from "./ClientCommandsManager.js";
import DBL from "@top-gg/sdk";
import express from "express";
const { Player } = discordPlayer, { Lyrics } = extractor;
/**
        ...
        if (this.config.topgg.enabled) {
            ...
            if (this.config.topgg.webhook.enabled) {
                ...
                await app.post("/dblwebhook", voteWebhook.listener(async (vote) => {
                    const channel = await thisSelf.channels.resolve(this.config.topgg.webhook.channelID);
                    ...
                    if (userdb.voteReminder) {
                        setTimeout(() => {
                            user.send("Hey! You can vote for me again! It's been 12 hours since you last voted. Click [here](https://top.gg/bot/803169312827113483/vote) to vote.");
                        }, 43200000);
                    }
                }));
