        ...
        message.channel.send({ embeds: [prompt] }).then(async (started) => {
            ...
            setSettings.on("collect", async (msg) => {
                if (msg.content.toLowerCase() === randomWord) {
                    gotWrong = false;
                    setSettings.stop();
                    return message.channel.sendSuccess(message, "Correct!", "You got it right!");
                }
                guessLimit = guessLimit - 1;
                if (guessLimit === 0) {
                    return setSettings.stop();
                }
                message.channel.sendError(message, "Wrong!", `That was wrong! You have ${guessLimit} guesses left!`);
            });
            setSettings.on("end", async () => {
                if (gotWrong)
                    return message.channel.sendError(message, "Thanks for playing.", `You either took too long or you ran out of guesses. The word was **${randomWord}**`);
            });
        });
    },
});
