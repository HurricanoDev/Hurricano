...
import Menu from "../../utilities/ButtonMenu.js";
...
function getEmoji(emoji) {
    let emote;
    if (!isNaN(emoji))
        emote = client.emojis.cache.get(emoji.toString());
    else
        emote = emoji;
    return emote;
}
function splitEmoji(emojiRaw) {
    ...
    if (/\p{Extended_Pictographic}/u.test(EmojiID))
        return EmojiID;
...
export default new Command({
    ...
    async run({ message, args }) {
        ...
        if (args.length) {
            ...
            if (!cmd)
                return message.sendErrorReply("Invalid Command Provided.", "I could not find the command you provided.", "Provide a valid command next time, smh.");
            ...
            if (Array.isArray(cmd.usage))
                ...
                emb.addField("Usage", "No usage provided.", true);
            if (Array.isArray(cmd.aliases))
                emb.addField("Aliases", cmd.aliases.map((alias) => `\`${pref}${alias}\``).join(" "), true);
            else
                emb.addField("Aliases", "No aliases provided.", true);
            if (Array.isArray(cmd.usage))
                emb.addField("Examples", cmd.examples.map((ex) => `\`${pref}${ex}\``).join("\n"), true);
            else
                emb.addField("Examples", "No examples provided.", true);
            emb.setFooter("Copyright Hurricano™");
            if (cmd.slash?.isSlash)
                emb.addField("Slash", "Yes.", true);
            else
                emb.addField("Slash", "No.", true);
            if (cmd.slash?.isNormal)
                emb.addField("Both Slash And Normal?", "Yes.", true);
            else
                emb.addField("Both Slash And Normal?", "No.", true);
            message.channel.send({ embeds: [emb] });
        }
        else {
            let cmdmap = {};
            client.commands.forEach((command) => {
                if (!cmdmap[command.category])
                    cmdmap[command.category] = [];
                if (command.slash.isSlash && !command.slash.isNormal)
                    cmdmap[command.category].push(`\`[Slash Command] ${command.name}\`, `);
                if (!command.slash.isSlash)
                    cmdmap[command.category].push(`\`${command.name}\`, `);
                if (command.slash.isSlash && command.slash.isNormal)
                    cmdmap[command.category].push(`\`[Slash And Normal Command] ${command.name}\`, `);
            });
            const emojimap = {
            ...
            const main = new MessageEmbed()
                ...
                .setColor("#ffb6c1")
                .addField(`> ${getEmoji(emojimap.config)} Config`, "The commands meant to modify the bot.", inline)
                .addField(`> ${getEmoji(emojimap.information)}  Information`, "Pretty self-explanitory! This module is meant for information commands.", inline)
                .addField(`> ${getEmoji(emojimap.fun)}  Fun`, "Commands in which you're sure to have fun!", inline)
                .addField(`> ${getEmoji(emojimap.giveaways)}  Giveaways`, "Host giveaways with Hurricano™️!", inline)
                .addField(`> ${getEmoji(emojimap.image)}  Image Manipulation`, "Make funny images with Hurricano™️!", inline)
                .addField(`> ${getEmoji(emojimap.moderation)}  Moderation`, "Let Hurricano™️ help the moderators and admins with its moderation system!", inline)
                .addField(`> ${getEmoji(emojimap.levelling)}  Levelling`, "Commands for Hurricano™️'s levelling module!", inline)
                .addField(`> ${getEmoji(emojimap.music)}  Music`, "Feel like listening to some music? You can do it with Hurricano™️!", inline)
                .addField(`> ${getEmoji(emojimap.utility)}  Utility`, "Want some handy tools? Well, here you go!", inline)
                .addField(`> ${getEmoji(emojimap.suggestions)} Suggestions`, "Commands related to the server suggestions system!", true)
                .addField(`> ${getEmoji(emojimap.economy)}  Economy`, "Wanna earn some of my money? Try out my economy system!", inline);
            client.config.ownerIds.includes(message.author.id)
                ? main.addField(`> ${emojis.categories.owner}  Owner`, "Commands meant for the bot owners.", inline)
                : null;
            main.addField(":bulb:  Fact:", `**${Fact}**`).setFooter("Copyright Hurricano™");
