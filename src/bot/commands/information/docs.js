...
export default new Command({
    ...
    async run({ message, args }) {
        ...
        switch (args[0]) {
            ...
            case "discordjs":
                if (!args[1])
                    return message.reply("Please provide what you would like to check docs of in discord.js!");
                let msg;
                ...
                if (!embed)
                    return message.reply("Nothing were found for your query in the discord.js docs.");
                msg = await message.reply({ embeds: [embed] });
                msg.react("🗑️");
                let confirmation = await msg
                    .awaitReactions({
                    filter: (reaction, user) => reaction.emoji.name === "🗑️" &&
