...
const decancer = weirdToNormalChars.weirdToNormalChars;
export default new Command({
    ...
    async run({ message, args }) {
        ...
        if (!target)
            return message.channel.sendError(message, "Error", "Invalid user provided!");
        ...
        if (nick === origiName)
            return message.channel.sendError(message, "Error!", "Target has already been decancered.");
