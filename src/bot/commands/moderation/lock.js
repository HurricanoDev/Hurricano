...
export default new Command({
    ...
    async run({ message, args }) {
        ...
        if (validUnlocks.includes(time)) {
            ...
            message.sendSuccessReply("Success.", `Successfully lifted the lock in ${channel}.`);
        }
        else {
            await channel.createOverwrite(message.guild.id, {
                SEND_MESSAGES: false,
            ...
            const lockdownembed = new Discord.MessageEmbed()
                ...
                .setFooter(`To unlock, use '${await message.client.db.guilds.getPrefix(message.guild.id)}lock unlock'`)
                .setColor("36393e");
            if (time != "none") {
                lockdownembed.addField("Locked for", ms(time, { long: true }), true);
            }
            await channel.send({ embeds: [lockdownembed] });
            await message.sendSuccessReply("Success.", `Successfully locked ${channel}.`, `To unlock it, use '${await message.client.db.guilds.getPrefix(message.guild.id)}lock unlock ${channel.name}'`);
            if (time !== "none") {
