...
export default new Command({
    ...
    async run({ message, args }) {
        ...
        if (!user)
            return message.sendErrorReply("Invalid User!", "Invalid user provided! Please provide a valid user.");
        let userSchema = await client.db.users.cache.get(user.id);
        if (!userSchema)
            userSchema = await client.utils.createUserDB(user);
        await message.channel.sendSuccess(message, "Confirmation.", `Are you sure you would like to unblacklist ${user}? Please respond with \`yes\` or \`no\`.`);
        let confirmation = await message.channel
            ...
            .catch(() => {
            return message.channel.sendError(message, "Time Limit Reached.", "You took too long to respond. You can try again later.");
        });
        ...
        if (negativeResponses.includes(confirmation.content.toLowerCase()))
            return await message.channel.sendSuccess("Cancelling Blacklist.", "Cancelling blacklist.");
        let positiveResponses = ["yes", "yep", "sure", "yessir"];
        if (positiveResponses.includes(confirmation.content.toLowerCase())) {
            message.sendSuccessReply("Unblacklisting...", "Unblacklisting that user now.");
