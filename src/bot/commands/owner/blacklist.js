...
export default new Command({
    ...
    async run({ message, args }) {
        ...
        if (!args.length) {
            message.sendSuccessReply("Cool!", `Cool, you would like to blacklist a user/guild!
       ...
       \`${prefix}blacklist user @Dɾαɠσɳιȥҽԃριȥȥα\``);
            ...
            if (conf === "guild" || conf === "server") {
                ...
                return message.channel.sendError(message, "Invalid Response Provided.", "You did not provide a valid response. Please try this command again.");
        }
        else {
            if (!types.includes(args[0].toLowerCase()))
                return message.channel.sendError(message, "Invalid Type Provided.", "Please provide whether you would like to blacklist a guild or user!");
            if (args[0] == "guild")
                toBlUser = false;
        }
        if (!args[1]) {
            message.channel.sendSuccess(message, "Cool!", `Now, please provide the ID of the ${toBlUser ? "user." : "guild."}`);
            let conf = await message.channel
                .awaitMessages({
                filter: (x) => x.author.id === message.author.id,
                max: 1,
                ...
                .catch(() => {
                return message.channel.sendError(message, "Time Limit Reached.", "You took too long to respond.");
            });
            conf = conf.first();
            if (toBlUser)
                user =
        ...
        if (toBlUser && !user)
            return message.channel.sendError(message, "Invalid User Provided.", "You provided an invalid user to blacklist. Please try this command again with a valid user!");
        else if (!toBlUser && !guild)
            return message.channel.sendError(message, "Invalid Guild Provided.", "You provided an invalid guild to blacklist. Please try this command again with a valid guild!");
        if (toBlUser) {
            message.channel.sendSuccess(message, "Confirmation.", `Are you sure you want to blacklist ${user.tag}?`, "Type yes if you want to, and no if you want to cancel.", null, [
                {
                    name: "User Data:",
          ...
          Mention: ${user}.`,
            ...
            if (confir === "yes") {
                ...
                return message.channel.sendSuccess(message, "Cancelling Blacklist.", `I am not blacklisting ${user}.`);
        }
        else {
          ...
          Owner Mention: ${guildOwner},`,
            ...
            if (confir === "yes") {
                ...
                return message.channel.sendSuccess(message, "Cancelling Blacklist.", `I am not blacklisting ${guild}.`);
        }
    },
});
