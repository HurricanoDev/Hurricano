import sourcebin from "../../utilities/SourcebinPost.js";
    ...
    async run({ message, args }) {
        async function sendEmbed(content, input, typeofOut) {
            ...
            if (content.length > 2032) {
                ...
                const msg = await message.author.sendSuccess(message, "Eval Output.", `The output for the eval command was larger than 2032 characters. To check it, click [here](${src.url}) or use the link: ${src.url}.`);
        ...
        if (!types.includes(args[0]))
            return message.channel.sendError(message, "Invalid Arguments Provided!", `Please provide if you would like to eval in \`sync, or async\`. \n Examples: \`${pref}eval sync <code>\`, \n \`${pref}eval async <code>\`.`);
        let code = args.map((x) => x);
        code.shift();
        code = code.join(" ");
        try {
            if (!code)
                return message.channel.sendError(message, "Invalid Arguments Provided!", "Please provide what you would like to eval!");
            let evaled;
            args[0] === "sync"
                ? (evaled = eval(code))
        ...
        ${code}
            ...
            if (evaled.includes(config.token) ||
                evaled.includes(config.mongouri))
                return message.channel.sendError(message, "Eval Error.", "This eval has the bot credentials! Please try without using the bot's credentials.");
            ...
            let conf = await msg
                ...
                time: 45000,
                filter: (x) => client.config.ownerIds.includes(x.user.id) &&
                    x.customId == "toDelete",
            })
                .catch(() => {
                msg.edit({ components: [] });
