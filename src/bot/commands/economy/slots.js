...
export default new Command({
    ...
    async run({ message, args }) {
        ...
        if (userInfo.wallet === 0)
            return message.channel.sendError(message, "Ha!", "Why are you trying to play the slot machine with no money? You'll have minus dollars after that!");
        let topEmojis = [
        ...
        if (amount < 0)
            embed.setDescription(`You lost **🪙 ${amount}**`);
        if (amount > 0)
            embed.setDescription(`You won **🪙 ${amount}**`);
        if (amount > 0)
            userInfo.wallet = userInfo.wallet + amount;
        if (amount < 0)
            userInfo.wallet = userInfo.wallet + amount;
        await userInfo.save();
