...
export default new Command({
    ...
    async run({ message, args }) {
        ...
        if (!colour)
            return message.sendErrorReply("Invalid Colour!", "Please specify a valid color.\n**Available Colors:**\n`Red [1.5x]`\n`Black [2x]`\n`Green [15x]`");
        colour = colour.toLowerCase();
        if (!money)
            return message.sendErrorReply("Error!", `Incorrect Usage. Usage: \`${message._usedPrefix}roulette <color> <amount>\``);
        if (money > moneyDB)
            return message.sendErrorReply("Error!", `The amount you provided is more than how much money you have in your wallet. Current Balance = 🪙 **${parseInt(moneyDB)}**`);
