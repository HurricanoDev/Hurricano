...
export default new Command({
    ...
    async run({ message, args }) {
        ...
        message.channel.send({
            embeds: [
                new MessageEmbed()
                    .setAuthor(`Search Results for ${args.join(" ")}.`, "https://raw.githubusercontent.com/HurricanoBot/HurricanoImages/master/SetAuthorEmojis/Success.png")
        ...
        collector.on("collect", (msg) => {
            ...
            if (!+msg.content)
                return message.channel.sendError(message, "Invalid Arguments.", "Please provide a valid number.");
            if (+msg.content <= 0)
                return message.channel.sendError(message, "Invalid Number Provided.", "Please provide a positive number.");
            if (+msg.content > tracks.length)
                return message.channel.sendError(message, "Invalid Number Provided.", `The number you provided is larger than the number tracks the search returned (${tracks.length + 1}).`);
