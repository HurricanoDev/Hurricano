...
export default new Command({
    ...
    async run({ message }) {
        const guildSchema = message.guild.db.cache(), types = [
            "messageLogs",
            "suggestionChannel",
            "systemChannel",
            "autoRole",
            ...
            "memberLog",
            "serverLog",
        ], roleSettings = ["autoRole", "muteRole"], otherSettings = ["antiSpam"], mainEmbed = new MessageEmbed()
        ...
        otherSettings.forEach((type) => {
            ...
            let title = type.toString().toLowerCase();
            title = client.utils.capitalize(title);
            Page4.addField(title, status ? `\`${client.utils.capitalize(desc)}\`` : "None");
        });
        //Buttons
        const leftButton = new MessageButton()
            .setStyle("PRIMARY")
            .setCustomId("DBLeft")
            .setEmoji("<:LeftArrow:861058356357365770>"), rightButton = new MessageButton()
            .setStyle("PRIMARY")
            .setCustomId("DBRight")
            .setEmoji("<:RightArrow:861058283657494559>"), msg = await message.channel.send({
            embeds: [mainEmbed],
            components: [[leftButton, rightButton]],
        });
        //Work
        currentPage = 1;
        const Pages = { 1: mainEmbed, 2: Page2, 3: Page3, 4: Page4 }, collector = msg.createMessageComponentCollector({
            idle: 30000,
            errors: ["time"],
