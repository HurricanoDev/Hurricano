...
export default new Command({
    ...
    async run({ message, args }) {
        ...
        let channel = guildSchema.suggestionChannel &&
            !isNaN(guildSchema.suggestionChannel)
            ? await client.channels.fetch(guildSchema.suggestionChannel)
            : undefined;
        if (!channel)
            return message.sendErrorReply("An Error Occured.", `This server does not have any suggestion channel! Ask an admin to set it up via: \n\`${prefix}suggestionchannel set {channel name}\``);
        ...
        await message.channel.sendSuccess(message, "Success!", `Successfully sent a suggestion! You can check it [here](${suggestionSent.url}).`);
    },
});
