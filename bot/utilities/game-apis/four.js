...
class Connect4 {
	...
	filter(reaction, user, userTurn) {
		return Object.keys(reactions).includes(reaction.emoji.name) && !user.bot && user.id == userTurn.id;
	}
	...
	hasWon(placedX, placedY) {
		...
		for (var i = Math.max(0, placedX - 3); i <= placedX; i++) {
			var adj = i + y;
			if (i + 3 < WIDTH) {
				if (gameBoard[adj] === chip && gameBoard[adj + 1] === chip && gameBoard[adj + 2] === chip && gameBoard[adj + 3] === chip) return true;
			}
		}
	...
	isBoardFull() {
		for (let y = 0; y < HEIGHT; y++) for (let x = 0; x < WIDTH; x++) if (gameBoard[y * WIDTH + x] === "⚪") return false;
		return true;
	}
