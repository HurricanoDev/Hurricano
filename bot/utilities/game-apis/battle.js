...
async function createBattle(member, message) {
	...
	if (member.user.bot)
		return message.sendErrorReply(
			"Error!",
			"You cannot battle against bots!",
		);
	...
	async function start(member, message) {
		...
		const confir = msg.createMessageComponentCollector();
		let rejectedOof;
		confir.on("collect", (button) => {
			if (
				button.user.id !== message.author.id &&
				button.user.id !== member.id
			)
				return button.reply({
					ephemeral: true,
			...
			if (button.user.id === message.author.id)
				return button.reply({
					content:
						"The person you are battling must confirm, not you!",
					ephemeral: true,
				});
			...
			for (i = 0; i < attacks.length; i++) {
				if (msg.content.toLowerCase() === attacks[i]) {
					let attackAmount =
						Math.floor(
							Math.random() *
								(settings.attackMax - settings.attackMin),
						) + settings.attackMin;
					let attackChance = Math.floor(
			...
			for (x = 0; x < heals.length; x++) {
				if (msg.content.toLowerCase() === heals[x]) {
					let healAmount =
						Math.floor(
							Math.random() *
								(settings.healMax - settings.healMin),
						) + settings.healMin;
	...
	async function end(message, member, winner) {
		...
		const emb16 = new MessageEmbed()
			...
			.setFooter(
				...
				won.username
					? won.displayAvatarURL()
					: won.user.displayAvatarURL(),
			);
		return message.channel.send({
			embeds: [emb16],
		});
