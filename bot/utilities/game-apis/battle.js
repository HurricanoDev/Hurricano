...
async function createBattle(member, message) {
	...
	if (member.id === message.author.id) return message.sendErrorReply("Error.", "You can't play with yourself!");
	...
	async function start(member, message) {
		const msg = await message.channel.sendSuccess(
			...
			[
				[
					new MessageButton().setStyle("SUCCESS").setCustomId("yes").setEmoji("✅"),
					new MessageButton().setStyle("DANGER").setCustomId("no").setEmoji("❌"),
				],
			],
		);
		...
		confir.on("collect", (button) => {
			...
			if (button.customId === "yes") {
				button.deferUpdate();
				msg.editSuccess("Accepted!", `${member} accepted your request, ${message.author}! Battle!`);
				confir.stop();
				...
				rejectedOof = true;
				confir.stop();
				return msg.editError("Rejected!", `Oof. ${member} rejected your request for battle.`);
			}
		});
		if (rejectedOof) return;
		setTimeout(() => {
			if (playerOneData.battleActive === false && playerOneData.battleActive === false) {
				return msg.editError("Timeout Reached.", "Oof, they didn't react in time.");
			}
		}, 15000);
	}
	async function accept(member, message) {
		...
		message.channel.send({ embeds: [battleEmbed] }).then(async (started) => {
			...
			setSettings.on("collect", async (msg) => {
				...
				if (args[0] === "heal" && args[1] && args[2] && !isNaN(args[2] && args[2] <= 200 && args[2] >= 2)) {
					if (args[1] === "min") {
				...
				} else if (args[0] === "attack" && args[1] && args[2] && !isNaN(args[2]) && args[2] <= 200 && args[2] >= 2) {
					if (args[1] === "min") {
				...
				} else if (args[0] === "health" && !isNaN(args[1]) && args[1] <= 500 && args[1] >= 50) {
					...
					const emb7 = new MessageEmbed()
						.setTitle("Battle!")
						.setDescription(
							`First Player: ${message.author} \`HP: ${playerOneData.battleHealth}\`\nSecond Player: ${member} \`HP: ${
	...
	async function first(member, message) {
		...
		collector.on("collect", async (msg) => {
			if (msg.content.toLowerCase() === "--end" || msg.content.toLowerCase() === "suicide") {
				let winner;
				if (playerOneData.battleHealth >= playerTwoData.battleHealth) winner = message.author.id;
				if (playerOneData.battleHealth < playerTwoData.battleHealth) winner = member.id;
			...
			for (i = 0; i < attacks.length; i++) {
				if (msg.content.toLowerCase() === attacks[i]) {
					let attackAmount = Math.floor(Math.random() * (settings.attackMax - settings.attackMin)) + settings.attackMin;
					...
					if (chance[attackChance] === "yes") {
						...
						const emb8 = new MessageEmbed()
							...
							.setColor("RED")
							.setAuthor(`Attack by: ${nowBattling.displayName}`, nowBattling.user.displayAvatarURL())
							.setFooter(`Next up: ${nextUp.displayName}`, nextUp.user.displayAvatarURL());
			...
			for (x = 0; x < heals.length; x++) {
				if (msg.content.toLowerCase() === heals[x]) {
					let healAmount = Math.floor(Math.random() * (settings.healMax - settings.healMin)) + settings.healMin;
					...
					if (chance[healChance] === "yes") {
						...
						const emb10 = new MessageEmbed()
							...
							.setColor("GREEN")
							.setAuthor(`Heal by: ${nowBattling.displayName}`, nowBattling.user.displayAvatarURL())
							.setFooter(`Next up: ${nextUp.displayName}`, nextUp.user.displayAvatarURL());
						await message.channel.send({
							embeds: [emb10],
						});
	...
	async function second(member, message) {
		...
		collector.on("collect", async (msg) => {
			if (msg.content.toLowerCase() === "--end" || msg.content.toLowerCase() === "Ã°Å¸ÂÂ³Ã¯Â¸Â" || msg.content.toLowerCase() === "suicide") {
				let winner;
				if (playerOneData.battleHealth >= playerTwoData.battleHealth) winner = message.author.id;
				if (playerOneData.battleHealth < playerTwoData.battleHealth) winner = member.id;
				await collector.stop();
	...
	async function end(message, member, winner) {
		...
		const emb16 = new MessageEmbed()
			...
			.setDescription(`${won} has won the battle with \`${wonData.battleHealth}\` HP Left!`)
			.setColor("GREEN")
			.setFooter(
