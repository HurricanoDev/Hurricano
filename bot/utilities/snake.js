...
class SnakeGame {
	...
	newGame() {
		...
		this.snake = [{ x: 5, y: 5 }];
		this.newAppleLoc();
		const embed = new Discord.MessageEmbed().setColor("#03ad03").setTitle("Snake Game").setDescription(this.gameBoardToString()).setTimestamp();
		...
		[
			...
			let button;
			if (em[0] == "⬛") {
				button = new Discord.MessageButton().setStyle("SECONDARY").setCustomId("noneRequired").setEmoji("⬛").setDisabled(true);
			} else if (em[0] == "❌") {
				button = new Discord.MessageButton().setStyle("DANGER").setCustomId("stop").setLabel("Stop.");
			} else {
				button = new Discord.MessageButton().setStyle("PRIMARY").setCustomId(em[1]).setEmoji(em[0]);
			}
			array.push(button);
		});
		array = split(array, 3);
		this.message.channel.send({ embeds: [embed], components: array }).then((emsg) => {
			this.gameEmbed = emsg;
	...
	waitForReaction() {
		...
		collector.on("end", (x) => {
			if (x.size && x.size == 0) {
				this.message.sendErrorReply("Ended.", "This game has ended due to inactivity.");
				this.gameOver();
			}
		});
	}
