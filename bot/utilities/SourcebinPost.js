...
const create = async (bins, options = {}) => {
	if (!bins || !Array.isArray(bins)) throw new TypeError("Invalid bins array provided.");
	...
	for (var x = 0; x < bins.length; x++) {
		...
		if (!bin.content || typeof bin.content !== "string") throw new TypeError("Invalid bin content provided.");
