...
const create = (bins, options = {}) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (!bins || !Array.isArray(bins)) return reject(new TypeError('Invalid bins array provided.'));
        ...
        for (var x = 0; x < bins.length; x++) {
            ...
            if ((typeof bin !== 'object')) return reject(new TypeError('Invalid bin provided.'));
            if (!bin.content || (typeof bin.content !== 'string')) return reject(new TypeError('Invalid bin content provided.'));
        ...
        fetch("https://sourceb.in/api/bins/", {
            ...
            headers: {
                ...
                'User-Agent': 'Sourcebin Post, By https://github.com/HurricanoBot/Hurricano'
            }
        }).then(res => {
            console.log(res)
            if (res.ok) {
