...
module.exports.Menu = class extends EventEmitter {
  ...
  constructor (channel, userID, pages, ms = 180000) {
    ...
    if (this.channel.type !== 'dm') {
      ...
      if (missingPerms.length) this.channel.send(`Looks like you haven't given the client ${missingPerms.join(', ')} permissions in #${this.channel}. This perm is needed for basic menu operation. You'll probably experience problems using menus in this channel.`)
    } else {
  ...
  start () {
    ...
    this.channel.send(this.currentPage.content).then(menu => {
      ...
      if (this.channel.type === 'dm') {
        ...
        this.author.send(`${error.toString()} (whilst trying to send menu message) | You're probably missing 'SEND_MESSAGES' or 'EMBED_LINKS' in #${this.channel.name} (${this.channel.guild.name}), needed for sending the menu message.`)
      }
    })
  }
    ...
    if (this.menu) {
      return this.menu.reactions.removeAll().catch(error => {
        if (this.channel.type === 'dm') {
          ...
          this.channel.send(`${error.toString()} (whilst trying to remove message reactions) | You've probably not given the client 'MANAGE_MESSAGES' permissions in #${this.channel}, needed for removing reactions when changing pages.`)
        }
      })
    }
  }
    ...
    for (const reaction in this.currentPage.reactions) {
      this.menu.react(reaction).catch(error => {
        if (error.toString().indexOf('Unknown Emoji') >= 0) {
          ...
          this.channel.send(`${error.toString()} (whilst trying to add reactions to message) | You're probably missing 'ADD_REACTIONS' in #${this.channel.name} (${this.channel.guild.name}), needed for adding reactions to the page.`)
        }
      })
    }
  }
