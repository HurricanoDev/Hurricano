...
module.exports.Menu = class extends EventEmitter {
	 ...
	 * @param {String} user The  of the user you want to let control the menu.
	...
	constructor(channel, author, pages, ms = 180000) {
		...
		if (!this.channel) {
			this.channel.client.users.cache.get(this.user).createDM(true);
		}
		if (this.channel.type !== "dm") {
			...
			if (missingPerms.length)
				this.channel.send(
					`Looks like you haven't given the client ${missingPerms.join(", ")} permissions in #${
						this.channel
					...
					this.author
						.send(
							`${error.toString()} (whilst trying to send menu message) | You're probably missing 'SEND_MESSAGES' or 'EMBED_LINKS' in #${
								this.channel.name
							} (${this.channel.guild.name}), needed for sending the menu message.`,
						)
						.catch(() => {});
				}
			});
	...
	clearReactions() {
		if (this.menu) {
			return this.menu.reactions.removeAll().catch((error) => {
				if (this.channel.type === "dm") {
					throw new Error(`Error due to sending ReactionMenu in DMs: ${error.toString()}.`);
				} else {
					this.channel.send(
			...
			this.menu.react(reaction).catch((error) => {
				if (error.toString().indexOf("Unknown Emoji") >= 0) {
					throw new Error(
						...
						}" (${reaction}) probably doesn't exist. You probably entered the  wrong when adding a custom emoji.`,
					);
				} else {
					this.channel.send(
						`${error.toString()} (whilst trying to add reactions to message) | You're probably missing 'ADD_REACTIONS' in #${
							this.channel.name
						} (${this.channel.guild.name}), needed for adding reactions to the page.`,
					);
				}
			});
		}
	...
	awaitReactions() {
		...
		this.reactionCollector.on("end", (reactions) => {
			...
			if (reactions.first()) {
				reactions.first().users.remove(this.menu.client.users.cache.get(this.user));
			}
		});

		this.reactionCollector.on("collect", (reaction, user) => {
			// If the name exists, prioritise using that, otherwise, use the . If neither are in the list, don't run anything.
			const reactionName = Object.prototype.hasOwnProperty.call(this.currentPage.reactions, reaction.emoji.name)
				? reaction.emoji.name
				: Object.prototype.hasOwnProperty.call(this.currentPage.reactions, reaction.emoji.id)
				? reaction.emoji.id
				: null;

			// If a 3rd party tries to add reactions or the reaction isn't registered, delete it.
			if (user.id !== this.author.id || !Object.keys(this.currentPage.reactions).includes(reactionName)) {
				...
				switch (this.currentPage.reactions[reactionName]) {
					...
					case "last":
						sameReactions =
							...
							JSON.stringify(Object.keys(this.pages[this.pages.length - 1].reactions));
					...
					case "previous":
						if (this.pageIndex > 0) {
							sameReactions =
								JSON.stringify(this.menu.reactions.cache.keyArray()) ===
								JSON.stringify(Object.keys(this.pages[this.pageIndex - 1].reactions));
					...
					default:
						sameReactions =
							JSON.stringify(this.menu.reactions.cache.keyArray()) ===
							JSON.stringify(Object.keys(this.pages.find((p) => p.name === this.currentPage.reactions[reactionName]).reactions));
