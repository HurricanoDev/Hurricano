...
module.exports.Menu = class extends EventEmitter {
	...
	constructor(channel, author, pages, ms = 180000) {
		...
		pages.forEach((page) => {
			...
			this.pages.push(
				new Page(page.name, page.content, page.reactions, i),
			);
			i++;
		});
	...
	awaitReactions() {
		...
		this.reactionCollector.on("end", (reactions) => {
			...
			if (reactions.first()) {
				reactions
					.first()
					.users.remove(
						this.menu.client.users.cache.get(this.userID),
					);
			}
		});
		...
		this.reactionCollector.on("collect", (reaction, user) => {
			...
			if (reactionName) {
				if (
					typeof this.currentPage.reactions[reactionName] ===
					"function"
				) {
				...
				switch (this.currentPage.reactions[reactionName]) {
					case "first":
						sameReactions =
							JSON.stringify(
								this.menu.reactions.cache.keyArray(),
							) ===
							JSON.stringify(
								Object.keys(this.pages[0].reactions),
							);
						this.setPage(0);
					...
					case "last":
						sameReactions =
							...
							JSON.stringify(
								Object.keys(
									this.pages[this.pages.length - 1].reactions,
								),
							);
					...
					default:
						...
						this.setPage(
							this.pages.findIndex(
								(p) =>
									p.name ===
									this.currentPage.reactions[reactionName],
							),
						);
						break;
				}
