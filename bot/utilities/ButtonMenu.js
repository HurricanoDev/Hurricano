...
module.exports = class ButtonMenu {
	...
	static validation(message, options) {
		if (typeof message !== "object") throw new Error("Message object provided is not an object.");
	}
	async start() {
		let row = [];
		...
		emojiKeys.forEach((totalObj) => {
			...
			const button = new MessageButton().setCustomId(emojiId.toString()).setStyle("SUCCESS").setEmoji(emojiId.toString());
			row.push(button);
		});
