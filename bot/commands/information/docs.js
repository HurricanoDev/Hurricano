...
module.exports = new Command({
	...
	async run(message, args) {
		...
		if (!validDocs.includes(args[0])) return message.reply("Please provide a valid docs source! Currently valid sources: \n `discord.js`");
		switch (args[0]) {
			case "discord.js":
			case "djs":
			case "discordjs":
				...
				const embed = await fetch(`https://djsdocs.sorta.moe/v2/embed?src=${branch}&q=${encodeURIComponent(args[1])}`).then((f) => f.json());
