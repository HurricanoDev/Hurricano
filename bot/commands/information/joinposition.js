...
module.exports = new Command({
  ...
  async run(message, args) {
    ...
    const members = (await message.guild.members.fetch())
      ...
      .array();
      if (parseInt(args[0]) && member < 0 || parseInt(args[0]) && member > message.guild.memberCount) return message.channel.sendError(message, 'Invalid Number!', 'Please provide a valid number!');
    let position;
    if (!parseInt(member)) {
