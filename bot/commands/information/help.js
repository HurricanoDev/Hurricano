...
module.exports = new Command({
	...
	async run(message, args) {
		...
		if (args.length) {
			...
			if (typeof cmd.usage == "array") emb.addField("Usage", `\`${pref}${cmd.usage}\``, true);
			else emb.addField("Usage", "No usage provided.", true);
			if (typeof cmd.aliases == "array") emb.addField("Aliases", cmd.aliases.map((alias) => `\`${pref}${alias}\``).join(" "), true);
			else emb.addField("Aliases", "No aliases provided.", true);
			if (typeof cmd.examples == "array") emb.addField("Examples", cmd.examples.map((ex) => `\`${pref}${ex}\``).join("\n"), true);
			...
			function splitEmoji(emojiRaw) {
				const Emoji = emojis.categories[emojiRaw];

				if (/\p{Extended_Pictographic}/u.test(Emoji)) return Emoji;

				return Emoji.split(":")[2].split("<")[0].split(">")[0];
			}
			const emojimap = {
			...
			//-------------------------------------
			function generateHelpEmbed(AuthorName, AuthorURL, description, CategoryCommands, Image) {
			...
			let helpMenu = new Menu(message, main, {
				...
				[emojimap.owner]: client.config.ownerIds.includes(message.author.id) ? owner : null,
			});
			helpMenu.start();
		}
	},
