...
module.exports = new Command({
	...
	async run(message, args) {
		...
		if (validUnlocks.includes(time)) {
			...
			var liftedembed = new Discord.MessageEmbed().setTitle("🔒 Lockdown").setDescription("🔓 Lockdown lifted.").setColor("36393e");
			...
			await message.sendSuccessReply(
				...
				`To unlock it, use '${await message.client.db.guilds.getPrefix(message.guild.id)}lock unlock ${channel.name}'`,
			);
			if (time !== "none") {
