...
module.exports = new Command({
	...
	async run(message, args) {
		...
		if (!target) return message.channel.sendError(message, "Error", "Invalid user provided!");
		if (target.id === message.author.id) return message.sendErrorReply("Error!", "You can't decancer yourself!");
		if (target.roles.highest > message.member.roles.highest)
			return message.channel.sendError(message, "Error.", "You can't decancer someone higher than you!");
		if (message.guild.me.roles.highest < target.roles.highest)
			return message.channel.sendError(message, "Error.", "I cannot decancer this user.");
		const origiName = target.displayName;
		const nick = decancer(target.displayName);
		if (nick === origiName) return message.channel.sendError(message, "Error!", "Target has already been decancered.");

		//Actually decancering the nickname smh.
		target.setNickname(nick);
		message.sendSuccessReply("Nickname Decancered.", `${target}'s name has been decancered. \n \`${origiName}\` => \`${nick}\``);
