...
module.exports = new Command({
	...
	async run(message, args) {
		let user = message.mentions.users.first() || (await message.client.users.fetch(args[0]).catch((e) => {}));
		if (!user) return message.sendErrorReply("Invalid User!", "Invalid user provided! Please provide a valid user.");
