	...
	async run(message, args) {
		async function sendEmbed(content, input, typeofOut) {
			...
			let embed = new MessageEmbed()
				...
				.addField("Input:", `\`\`\`js\n${toEval.length > 1016 ? "Input is larger than 1016 characters." : toEval}\`\`\``, true)
        ...
        ${code}
			...
			const row = new MessageButton().setCustomId("toDelete").setLabel("Delete?").setStyle("DANGER");
			...
			let conf = await msg
				.awaitMessageComponentInteraction({
					time: 45000,
					filter: (x) => client.config.ownerIds.includes(x.user.id) && x.customId == "toDelete",
				})
				.catch(() => {
					msg.edit({ components: [] });
				});
