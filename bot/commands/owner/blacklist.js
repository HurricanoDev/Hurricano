	...
	async run(message, args) {
       ...
       \`${prefix}blacklist user @Dɾαɠσɳιȥҽԃριȥȥα\``,
		...
		if (!args[1]) {
			...
			conf = conf.first();
			if (toBlUser) user = conf.mentions.users.first() ?? (await client.users.fetch(conf.content).catch(() => {}));
			else guild = client.guilds.cache.get(conf.content);
		} else {
			if (toBlUser) user = message.mentions.users.first() ?? (await client.users.fetch(args[1]).catch(() => {}));
		...
		if (toBlUser) {
          ...
          Mention: ${user}.`,
			...
			const guildOwner = await client.users.fetch(guild.owner).catch(() => {});
