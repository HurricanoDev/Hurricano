...
module.exports = new Command({
	...
	async run(message, args) {
		try {
			...
			exec("git pull" || "date", function (err, stdout, stderr) {
				...
				const emSuccess = new MessageEmbed()
					...
					.setDescription(`\`\`\`xl\n${stdout.length > 2038 ? "Pull summary is larger than 2038 characters." : stdout}\n\`\`\``)
					.setTimestamp()
					.setColor(123456)
					.setFooter(`Requested by: ${message.author.tag}`);
				message.channel.stopTyping(true);
		...
		if (response.includes("yes")) {
			await message.channel.sendSuccess(message, "Rebooting...", "Rebooting the bot now.");
			setTimeout(() => {
				process.exit();
			}, 1000);
		}
		if (response.includes("no")) {
			await message.channel.sendSuccess(message, "Reboot Cancelled.", "Will not reboot the bot now.");
		}
	},
});
