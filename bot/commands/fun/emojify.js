...
module.exports = new Command({
	...
	async run(message, args) {
		if (!args.join(" ")) return message.channel.sendError(message, "Error!", `Correct Usage: \`${message._usedPrefix}emojify <text>\``);

		function hasNumber(myString) {
			return /\d/.test(myString);
		}

		if (hasNumber(args.join(" "))) return message.channel.sendError(message, "Error!", "I can't emojify text with numbers in it!");
