...
module.exports = new Command({
	...
	async run(message, args) {
		...
		const gws = this.client.giveawaysManager.giveaways.filter((g) => g.guild === message.guild.id).filter((g) => !g.ended);
		if (gws.length) {
			await Promise.all(
				gws.map(async (x) => {
					embed.addField(
        ...
        ${x.extraData.role == null ? "Required Role: " + message.guild.roles.cache.get(x.extraData.role).toString() : ""}.`,
						true,
					);
				}),
			);
