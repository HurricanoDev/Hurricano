...
module.exports = new Command({
	...
	async run(message, args) {
		...
		if (
			...
			!message.member.roles.cache.has((r) => r.name.toLowerCase() === "Giveaway Manager" && !client.config.ownerIds.includes(message.author.id))
		) {
		...
		if (client.giveawaysManager.giveaways.filter((g) => g.guild === message.guild.id && !g.ended).length + 1 > 10)
			return message.sendErrorReply(
				"An Error Occured.",
		...
		let winners = args[1] && args[1].toLowerCase().endsWith("w") ? +args[1].replace("w", "") : +args[1];
		if (!winners)
			return message.sendErrorReply(
				"An Error Occured.",
