...
module.exports = new Command({
	...
	async run(message, args) {
		if (
			...
			!message.member.roles.cache.includes((r) => r.name.toLowerCase().includes("Giveaway Manager"))
		) {
		...
		let hasGiveaway = message.client.giveawaysManager.giveaways.find((g) => g.message === id);
		if (!hasGiveaway)
			return message.reply({
				embeds: [
					{
		...
		message.client.giveawaysManager
			.end(hasGiveaway.message)
			.then(() => {
				message
					.reply("Giveaway will end in less than " + global.client.giveawaysManager.options.updateCountdownEvery / 1000 + " seconds...")
