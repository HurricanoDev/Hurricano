...
module.exports = new Command({
	...
	async run(message, args) {
		...
		avaliableModules.forEach((d) => modulesEmbed.addField(d, `\`${prefix}disable ${d}\``, true));
		...
		if (!avaliableModules.includes(argss)) {
			const invalidEmbed = new MessageEmbed().setTitle("Invalid Arguments Provided.").setDescription("Available Modules:");
			avaliableModules.forEach((d) => invalidEmbed.addField(d, `\`${prefix}disable ${d}\``));
		...
		if (avaliableModules.includes(argss)) {
			...
			if (disabledModules.includes(argss)) {
				message.channel.sendError(message, "Already Disabled.", "The module you provided is already disabled.");
				return;
			}
			disabledModules.push(argss);
			guildSchema.disabledModules = disabledModules;
			const data = await guildSchema.save();
			await message.channel.sendSuccess(message, "Module Enabled.", `The module \`${argss}\` has been disabled.`);
		}
	},
});
