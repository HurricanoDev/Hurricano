...
module.exports = new Command({
	...
	slash: {
		...
		async run(interaction, args) {
			...
			if (!interaction.channel.permissionsFor(interaction.user.id).has("ADMINISTRATOR") && args[1].value !== "list")
        ...
        \`\`\`${guildSchema.prefixes.join("\n")}\`\`\``);
				...
				case "remove":
					if (!args[1].value)
						return await interaction.reply("Please try this command again, with the prefix you would like to remove.", {
							ephemeral: true,
						});
      ...
      \`${message._usedPrefix}prefix remove ?\``,
		...
		switch (args[0].toLowerCase()) {
			case "add":
				...
				if (!args[1] === `<@!${client.user.id}>`) return message.channel.sendError(message, "Error", "I already have a mention prefix!");
