	...
	async run(message, args) {
		...
		switch (args[0].toLowerCase()) {
			case "set":
				...
				if (!SBChannel) return message.sendErrorReply("Error", "You need to give me a valid channel!");
				...
				break;
			case "remove":
				if (!guildData.starBoard) return message.sendErrorReply("Error!", "There is no existing starboard channel to remove!");
