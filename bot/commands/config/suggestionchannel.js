...
module.exports = new Command({
  ...
  async run(message, args) {
    ...
    if (!args.length)
      ...
      let channelSchema = await client.schemas.guild.findOne({ id: message.guild.id });
      switch(args[0]) {
    case 'set':
      const channel =
    ...
    if (!channel)
      ...
      if (suggestionChannel) return message.channel.sendError(message, 'Suggestions Channel Already Set!', `A suggestions channel has already been set! You can use \`${prefix}suggestionchannel remove\` to remove it.`);
      ...
      break;  
      case 'remove':
        if (!suggestionChannel) return message.channel.sendErrorReply(message, 'No Suggestions Channel Found!', `No suggestion channel was found! If you want to set one, you can use: \`${prefix}suggestionchannel set {channel}\`.`)
        await client.schemas.guild.findOneAndUpdate({
          id: message.guild.id
        }, {
