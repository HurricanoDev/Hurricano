...
module.exports = new Command({
	...
	async run(message, args) {
		...
		avaliableModules.forEach((d) => modulesEmbed.addField(d, `\`${prefix}enable ${d}\``, true));
		...
		if (!avaliableModules.includes(argss)) {
			...
			avaliableModules.forEach((d) => invalidEmbed.addField(d, `\`${prefix}enable ${d}\``));
		...
		if (avaliableModules.includes(argss)) {
			...
			if (!disabledModules.includes(argss)) {
				message.channel.sendError(message, "Already Enabled.", "The module you provided is already enabled.");
				return;
			}
			...
			await message.channel.sendSuccess(message, "Module Enabled.", `The module \`${argss}\` has been enabled.`);
		}
	},
});
