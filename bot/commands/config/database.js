...
module.exports = new Command({
	...
	async run(message, args) {
		...
		const types = ["messageLogs", "suggestionChannel", "systemChannel", "autoRole", "memberLog", "muteRole", "antiSpam", "serverLog"];
		const channelSettings = ["messageLogs", "suggestionChannel", "systemChannel", "memberLog", "serverLog"];
		...
		const mainEmbed = new MessageEmbed()
			...
			.setDescription(`**More information:** \`${message._usedPrefix}settings [category]\``)
			...
			.setFooter(`© Hurricano`);
		let currentPage;
		const Page2 = new MessageEmbed().setTitle("Page 2/4: **`Channel`**").setTimestamp().setColor("#6082b6").setFooter(`© Hurricano`);
		...
		const Page3 = new MessageEmbed().setTitle("Page 3/4: **`Role`**").setTimestamp().setColor("#6082b6").setFooter(`© Hurricano`);
		...
		const Page4 = new MessageEmbed().setTitle("Page 4/4").setTimestamp().setColor("#6082b6").setFooter(`© Hurricano`);
		otherSettings.forEach((type) => {
			...
			let title = type.toString().toLowerCase();
			title = client.functions.capitalize(title);
			Page4.addField(title, status ? `\`${client.functions.capitalize(desc)}\`` : "None");
		});

		//Buttons
		const leftButton = new MessageButton().setStyle("PRIMARY").setCustomId("DBLeft").setEmoji("<:LeftArrow:861058356357365770>");
		const rightButton = new MessageButton().setStyle("PRIMARY").setCustomId("DBRight").setEmoji("<:RightArrow:861058283657494559>");
