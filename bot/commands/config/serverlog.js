	...
	async run(message, args) {
		...
		switch (args[0].toLowerCase()) {
			case "set":
				...
				const serverLogChannel = message.mentions.channels.first() || message.guild.channels.cache.get(args[1]);

				if (!guildData.serverLog && !serverLog.viewable && !serverLog.permissionsFor(message.guild.me).has("SEND_MESSAGES"))
					return message.sendError(
						"Error",
