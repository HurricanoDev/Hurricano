...
module.exports = new Command({
  ...
  async run(message, args) {
    ...
    collector.on('collect', (msg) => {
      ...
      if (
        ['cancel', 'stop', 'end', 'exit'].includes(msg.content.toLowerCase())
      ) {
        collector.stop();
        return message.channel.sendSuccess(
          message,
          'Success.',
          'Successfully ended the search.',
        );
      }
      if (!+msg.content)
        return message.channel.sendError(
      ...
      if (+msg.content > tracks.length)
        return message.channel.sendError(
          message,
          'Invalid Number Provided.',
          `The number you provided is larger than the number tracks the search returned (${
            tracks.length + 1
          }).`,
        );
