	...
	if (cd) {
		...
		if (cd.expiration < now) {
			...
			return message.sendErrorReply(
				"Chillza.",
				`You need to wait ${Math.floor((cd.expiration - now) / 1000).toFixed(1)} more second(s) before reusing the \`${
					command.name
				}\` command.`,
...
module.exports = class MessageEvent extends BaseEvent {
	...
	async run(message, client) {
		...
		if (!message.channel.permissionsFor(client.user.id).has(["SEND_MESSAGES", "READ_MESSAGE_HISTORY", "EMBED_LINKS", "ADD_REACTIONS"]))
		...
		blacklistedWords.map((x) => {
			if (
				...
				!message.author.bot &&
				(!checkPerm("ADMINISTRATOR") || !checkPerm("MANAGE_GUILD") || !checkPerm("MANAGE_MESSAGES"))
			)
				deleting = true;
		});
		...
		const whichToUse = prefixes?.length == 1 ? guildSchema.prefixes.toString() : "{prefix/mention}";
		const embed = new MessageEmbed()
			...
			.addField("Forgot my prefix?", `No worries! My ${guildSchema.prefixes?.length > 1 ? "prefixes are" : "prefix is"} ${prefixes}.`)
			.addField(
				"Need help?",
		...
		if (prefixRegex.test(message.content.toLowerCase())) {
			...
			if (message.content === `<@${client.user.id}>` || (message.content === `<@!${client.user.id}>` && !userSchema.blacklisted))
			...
			if (!command) {
				const best = [...client.commands.map((cmd) => cmd.name), ...client.aliases.keys()].filter(
			...
			if (command.subCommands && args.length) {
				...
				if (subCommand) {
					...
					if (subCommands.baseAuthorization) {
						const auth = subCommands.baseAuthorization ? await subCommands.baseAuthorization(message, args, subArgs, args[0]) : null;
			...
			if (command.conf.userPermissions) {
				if (!checkPerm(command.userPermissions) && !client.config.ownerIds.includes(message.author.id)) {
					return message.sendErrorReply(
						"Permission Error.",
						`Stop disturbing me bro, you require the \`${command.userPermissions.join(", ")}\` permission(s) to use that command...`,
						"Smh, imagine trying to use a command without having the perms-",
					);
				}
			}
			if (command.conf.args && !args.length) return message.channel.sendError(message, "Arguments Error.", command.conf.args);
			...
			if (disabledModules && !disabledModules.includes("levelling")) {
				...
				if (!userLevel) await client.levels.createUser(message.author.id, message.guild.id);
			...
			if (!client.config.ownerIds.includes(message.author.id)) await makeCooldown(message, command);
			client.logger.message(
				`${message.author.tag} used the "${command.name}" command in guild ${message.guild} with args: "${args.join(" ")}"`,
			);
