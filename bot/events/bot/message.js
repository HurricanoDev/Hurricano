		...
		if (prefixRegex.test(message.content.toLowerCase())) {
			...
			if (!command) {
				const best = [
					...
					(c) =>
						leven(cmd.toLowerCase(), c.toLowerCase()) <
						c.length * 0.4,
				);
				const dym =
					best.length == 0
			...
			if (disabledModules && !disabledModules.includes("levelling")) {
				...
				if (!userLevel)
					await client.levels.createUser(
						message.author.id,
						message.guild.id,
					);
			...
			client.logger.message(
				`${message.author.tag} used the "${
					command.name
				}" command in guild ${message.guild} with args: "${args.join(
					" ",
				)}"`,
			);
