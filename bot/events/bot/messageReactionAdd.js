...
module.exports = class messageReactionAdd extends BaseEvent {
	...
	async run(reaction, user) {
		...
		const starBoardChannel = await client.channels.fetch(guildData.starBoard.channel).catch(() => {});
		if (!starBoardChannel) {
		...
		if (!starBoardChannel.permissionsFor(reaction.message.guild.me).has(["SEND_MESSAGES", "READ_MESSAGE_HISTORY"])) return;
		const starBoardMsgId = guildData.starBoard.messages.find((x) => x[0] == reaction.message.id);
		const sentMessage = starBoardMsgId ? await starBoardChannel.messages.fetch(starBoardMsgId[1]).catch(() => {}) : null;
		...
		if (!sentMessage && reaction.count < guildData.starBoard.minimumReactions) return;
		if (sentMessage) {
