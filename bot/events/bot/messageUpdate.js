...
module.exports = class messageUpdate extends BaseEvent {
	...
	async run(oldMessage, newMessage, client) {
		...
		const snipeObject = {
			...
			images:
				oldMessage.attachments.first() && oldMessage.attachments != newMessage.attachments
		...
		if (guildSchema.messageLogs) {
			const guildChannel = await client.channels.fetch(guildSchema.messageLogs).catch(() => {});
			if (!guildChannel) return;
			let embed = new MessageEmbed()
				.setAuthor(`Message Edited By ${oldMessage.author?.tag}! | (ID: ${oldMessage.author?.id})`, oldMessage.author.displayAvatarURL())
				.setDescription(
					`${
				...
				.setFooter(`Deleted by ${newMessage.author?.tag} | ${newMessage.author?.id}`)
				.setColor("#6082b6");
			oldMessage.attachments.first() && oldMessage.attachments != newMessage.attachments
				? (() => {
