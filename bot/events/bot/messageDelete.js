...
module.exports = class messageDeleteEvent extends BaseEvent {
	...
	async run(message, client) {
		...
		if (guildSchema.messageLogs) {
			const guildChannel = message.guild.channels.cache.get(guildSchema.messageLogs);
			let embed = new MessageEmbed()
				.setAuthor(`Message Deleted By ${message.author.tag}! | ID: ${message.author.id})`, message.author.displayAvatarURL())
				.setDescription(
					`${message.content.length > 2034 ? "Message content is larger than 2034 characters!" : "**Content:**\n" + message.toString()}`,
				)
