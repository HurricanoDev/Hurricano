...
table.setHeading("Command File", "Command Name", "Command Category", "Aliases", "Load status");
...
class HurricanoClient extends Discord.Client {
	...
	constructor(config, options = {}) {
		...
		this.progressChannel = this.channels.cache.get(this.config.botChannels.serverJoinChannel);
		...
		this.functions = {
			...
			getMember: async (returnAuthor, message, ...args) => {
				if (!returnAuthor && returnAuthor !== false) throw new Error(`Returning message.author not specified.`);
			...
			getChannel: async (returnChannel, message, ...args) => {
				if (!returnChannel && returnChannel !== false) throw new Error(`Returning message.author not specified.`);
				if (!message) throw new Error(`Message object not provided.`);
				if (!args.length) throw new Error(`Arguments string not provided.`);
				if (typeof returnChannel !== "boolean") throw new Error(`Whether to return channel or not option is not boolean.`);
				if (typeof message !== "object") throw new Error(`Message provided is not an object.`);
				args = args[0];
				let user;
			...
			createOptionsEmbed: (command, message) => {
				const optionsEmbed = new Discord.MessageEmbed()
					...
					.addField(
						"Permissions",
						command.userPermissions?.length ? command.userPermissions.map((x) => `\`${x}\``).join(", ") : "No user permissions required.",
					)
	...
	loadEvents() {
		// BOT EVENTS
		const botevents = readdirSync("./bot/events/bot").filter((file) => file.endsWith(".js"));
		...
		// MUSIC EVENTS
		const musicevents = readdirSync("./bot/events/music").filter((file) => file.endsWith(".js"));
		...
		// GIVEAWAYS EVENTS
		const giveawaysevents = readdirSync("./bot/events/giveaways").filter((file) => file.endsWith(".js"));
		for (const file of giveawaysevents) {
			const event = require(`./events/giveaways/${file}`);
			this.giveawaysManager.on(event.name, (...args) => event.run(...args, this));
		}
	}
	loadCommands() {
		readdirSync("./bot/commands").forEach((dir) => {
			...
			try {
				for (let file of commands) {
					...
					if (props.name) {
						table.addRow(file, props.name, dir, props.aliases ? props.aliases.join(", ") : "None.", "Loaded!");
					} else {
						table.addRow(
							file,
							props.name,
			...
			if (this.config.topgg.webhook.enabled) {
				...
				await app.post(
					...
					voteWebhook.listener(async (vote) => {
						const channel = await thisSelf.channels.resolve(this.config.topgg.webhook.channel);
						const user = this.users.resolve(vote.user);
						const embed = new Discord.MessageEmbed()
							.setAuthor(user.username, user.displayAvatarURL())
							.setDescription(`${user} just voted! Poggers! Sending them a thank message right now!`)
						...
						const dmEmbed = new Discord.MessageEmbed()
							...
							.setDescription(`Hey **${user.username}**, thanks so much for your vote! This helps us in unimaginable ways!`)
							.setThumbnail("https://raw.githubusercontent.com/HurricanoBot/HurricanoImages/master/other/VotePog.png")
							.setColor("GREEN");
...
function loadStructures() {
	const structures = readdirSync("./bot/structures/ImmediateExecute").filter((file) => file.endsWith(".js"));

	for (const file of structures) {
		const name = file.split(".")[0],
			struct = require("discord.js")[name];

		require("./structures/ImmediateExecute/" + name)({
			[name]: struct,
			merge(newObj) {
				recursiveShallowClone(struct.prototype, newObj);
			},
		});
	}
}
