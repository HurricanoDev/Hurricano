...
function replaceStringsInObject(objRaw, findStr, replaceStr) {
	for (const key in Object.getOwnPropertyNames(objRaw)) {
		...
		if (typeof value === "string") {
			...
			for (let i = 0; i < findStr.length; i++) str = str.replace(new RegExp(RegexEscape(findStr[i]), "g"), replaceStr[i]);

			objRaw[key] = str;
		} else if (typeof value === "object") replaceStringsInObject(value, findStr, replaceStr);
	}
}
...
module.exports = ({ merge }) =>
	merge({
		async send(optionsRaw) {
			...
			let options =
				typeof optionsRaw === "string"
					? optionsRaw.replace(tokenRegex, "Bot Token.").replace(mongoUri, "MongoDB Uri.")
					: replaceStringsInObject(optionsRaw, [tokenRegex, mongoUri], ["Bot Token.", "MongoDB Uri."]);
		...
		async sendError(message, header, msg, footer, fields, components) {
			...
			return this.send({
				embeds: [embed],
				components: components.map((component) => new MessageActionRow().addComponents(...component)),
			});
		},
		async sendSuccess(message, header, msg, footer, fields, components) {
			...
			return this.send({
				embeds: [embed],
				components: components.map((component) => new MessageActionRow(...component)),
			});
		},
	});
