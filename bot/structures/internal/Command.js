		...
		this.run =
			...
			function run() {
				if (!this.slash && !this.subCommands)
					throw new Error(
						`Command ${name} doesn't have a run method.`,
					);
			};
		this.slash = {
			...
			run:
				...
				function run() {
					throw new Error(
						`Command ${name} doesn't have a slash run method.`,
					);
				},
	...
	static validateOpts(opts) {
		...
		if (opts.aliases) {
			...
			if (opts.aliases.some((ali) => ali !== ali.toLowerCase()))
				throw new RangeError(
					`Command: ${name}: Aliases are not lowercase.`,
				);
		...
		if (opts.subCommands) {
			if (typeof opts.subCommands !== "object")
				throw new Error(
					`Command: ${name}: Subcommands list aren't an array.`,
				);
		...
		if (opts.ownerOnly && typeof opts.ownerOnly !== "boolean")
			throw new TypeError(
				`Command: ${name}: ownerOnly is not a boolean.`,
			);
	}
};
