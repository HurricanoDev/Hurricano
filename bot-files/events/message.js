module.exports = async (client, message) => {
        ...
        await mongoconnect().then(async (mongoose) => {
          try {
            for (const guild of client.guilds.cache) {
              ...
              guildPrefixes[guildId] = result ? result.prefix : config.prefix
            }
          } finally {
  ...
  const prefixMention = new RegExp(`^<@!?${client.user.id}>`);
    ...
    .setDescription(`Hello! I'm **DragonNight™**. My prefix is \`${guildPrefixes[guildId]}\`. I have a variety of commands you can use! If you want to view information about me please do \`dn!info\`. That's it for now, bye and have a great time!`)
    .setColor("#034ea2")
 ...
 if (!message.content.startsWith(guildPrefixes[guildId])) return;
  const args = message.content.slice(guildPrefixes[guildId].length).trim().split(/ +/g);
